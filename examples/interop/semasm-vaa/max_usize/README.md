# max_usize — SemASM / VAA bridge leaf

Return the larger of two unsigned sizes. Export symbol `max_usize` matches
the SemASM contract `max_usize.sem.toml` and VAA task `max-usize-win64-v1`
(oracle `builtin.pure_int.binary_usize`, claim `max`).

## Emit Win64 NASM for VAA (shared-library — no `_start`)

```bash
hla64 emit-nasm examples/interop/semasm-vaa/max_usize/max_usize.hla64 \
  --target windows-x64-msabi \
  --output-kind shared-library \
  -o ../vaa/fixtures/ingest/hlax64_max_usize/candidate.asm
```

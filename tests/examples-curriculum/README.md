...
29 runnable Linux curriculum examples are covered. The seven `real-*` manifests compile-check Windows-focused daily-use tools; their native Windows execution is locked by `RealTool_builds_and_runs_natively_on_windows` in `HlaX64.AssemblyLab.Tests`, which reads each tool's `expected.stdout` and `expected.exitcode`.

...
public class NasmEmitterTests
{
    ...
    [Fact]
    public void Emit_SysVSignedGreaterThan_UsesAboveBranch()
    {
        const string source = """
            program max_signed;
    ...
    [Fact]
    public void Emit_Win64UnsignedLessThan_UsesBelowBranch()
    {
        const string source = """
            program min_unsigned;
            export procedure min_unsigned(a: int64; b: int64); @returns("rax");
    ...
    [Fact]
    public void Emit_Win64UnsignedGreaterThan_UsesAboveBranch()
    {
        const string source = """
            program max_unsigned;
            export procedure max_unsigned(a: int64; b: int64); @returns("rax");
    ...
    [Fact]
    public void Emit_SysVFramedProc_BalancesRbpPushPop()
    {
        // Regression for live-repair ABI_STACK_BALANCE_001 (SysV twin): framed
        // SysV procedures must restore rbp before ret (push rbp ↔ pop rbp).
    ...
    public void Emit_Win64FramedProc_DoesNotClobberRbxUnsaved()
    {
        ...
        // leaf must not write rbx without a matching push rbx.
        const string source = """
            program stack_local;
            export procedure stack_local(x: int64); @returns("rax");
            var

...
- **Advanced hardening round 2 (Batches A–F):** AVX2 YMM lowering (`vaddpd`, `vmovapd`, `vxorpd`); `simd.*` intrinsics + `atomic.*` MVP; path/git dependency resolver + lock verification on `build`; DAP server (`HlaX64.DebugAdapter`, Linux gdb); source map columns; semantic token modifiers; partial-parse signature help; SysV variadic `printf` (int + cstring); `hla64 test-differential` + CI; HLAX0072–0073; 308 tests
